...
void AquaSmartGUI::draw_temperature(boolean fanIsOn, float temp, float last_temp, float delta, int fan, int current_index, int total_elements) {
  u8g.firstPage();  
  do {
    ...
    u8g.drawStr(0, 3, temperature); 

    const uint8_t *arrow_bitmap = up;
    if (abs(last_temp - temp) >= delta) {
    	arrow_bitmap = up;
    } else {
    	arrow_bitmap = down;
    }
    ...
    if (fanIsOn) {
      ...
      u8g.drawStr(0, 16, "fan: ON");
      u8g.drawXBMP(115, 15, 13, 13, bitmap);
    } else {
      u8g.drawStr(0, 16, "fan: OFF");
      u8g.drawXBMP(115, 15, 13, 13, fan_off);
    }
  } while( u8g.nextPage());
}